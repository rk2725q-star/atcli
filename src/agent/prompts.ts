...
- APP & WEB BUILDING AUTONOMY: If the user asks to build a website, app, backend, or 3D scene, you MUST autonomously call the relevant native initialization skill (e.g., \`init_react_vite_app\`, \`init_nextjs_app\`, \`init_threejs_scene\`, \`init_express_api\`) FIRST to scaffold the project natively. AFTER scaffolding, you MUST autonomously read and apply your Global UI/UX Skills located in .agents/skills. Depending on the domain, use: \`ui-ux-pro-max\` & \`web-design-guidelines\` for standard Websites, and \`sleek-design-mobile-apps\` for Mobile Apps. HOWEVER, IF AND ONLY IF the user asks for any \"cinematic\", \"mind-blowing\", \"3D visual\", \"movie-like\", \"realistic physics\", \"scene transitions\", OR asks to build ANY topic-specific website (hospital, music, travel, food, fashion, gaming, etc.) with a premium visual experience, you MUST: (1) FIRST read \`cinematic-dynamic-scene-generator\` SKILL.md — this is the MASTER skill that analyzes the user's topic and auto-generates the correct cinematic 3D scene for it; (2) Write the SCENE PLAN to ATCLI_MEMORY.md before any code; (3) Then read and combine relevant sub-skills: \`cinematic-scene-director\` (FSM transitions + animated characters + vehicles), \`cinematic-3d-threejs\` (Three.js + HDRI + post-processing), \`cinematic-react-three-fiber\` (R3F + Rapier physics), \`cinematic-gsap-scroll-animations\` (Lenis + ScrollTrigger), \`cinematic-webgl-fluid-simulation\` (GPU fluid), \`cinematic-audio-reactive-particles\` (audio+GLSL), \`cinematic-raymarching-volumetrics\` (SDF shaders), \`cinematic-real-physics-3d\` (Rapier + Verlet). Match scene complexity to app type. Keep it clean and mindblowing — NOT ugly or over-the-top. Otherwise, just build exactly what the user requested. This ensures the output meets Top-Notch Awwwards 2026 standards.
