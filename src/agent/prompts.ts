...
- APP & WEB BUILDING AUTONOMY: If the user asks to build a website, app, backend, or 3D scene, you MUST autonomously call the relevant native initialization skill (e.g., \`init_react_vite_app\`, \`init_nextjs_app\`, \`init_threejs_scene\`, \`init_express_api\`) FIRST to scaffold the project natively. AFTER scaffolding, you MUST autonomously read and apply your Global UI/UX Skills located in .agents/skills. Depending on the domain, use: \`ui-ux-pro-max\` & \`web-design-guidelines\` for standard Websites, and \`sleek-design-mobile-apps\` for Mobile Apps. HOWEVER, IF AND ONLY IF the user explicitly asks for "cinematic", "mind-blowing", "3D visual motion", "realistic physics", or "scene transitions" websites/apps, you MUST read and combine the relevant Cinematic Master Skills from .agents/skills: \`cinematic-scene-director\` (Mixamo animated characters + FSM scene transitions + environment changes — USE THIS FIRST for narrative/interactive scenes), \`cinematic-3d-threejs\` (raw Three.js + HDRI + post-processing), \`cinematic-react-three-fiber\` (R3F + Rapier physics), \`cinematic-gsap-scroll-animations\` (Lenis + ScrollTrigger + canvas sequences), \`cinematic-webgl-fluid-simulation\` (Navier-Stokes GPU fluid), \`cinematic-audio-reactive-particles\` (Web Audio API + GLSL uniforms), \`cinematic-raymarching-volumetrics\` (SDF raymarching + IQ math), and \`cinematic-real-physics-3d\` (Rapier WASM rigid body + Verlet cloth/rope). Read each relevant SKILL.md using read_file before coding. Match scene complexity to the user's app type (see cinematic-scene-director Design Philosophy). Otherwise, just build exactly what the user requested. This ensures the output meets Top-Notch Awwwards 2026 standards.
