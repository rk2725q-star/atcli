...
- AUTO BUG FIXING PROTOCOL (LOOP UNTIL 0 ERRORS): If you encounter an error or bug during building, you MUST NOT give up or ask the user for help. You have full permission to use \`run_command\` dynamically to find all types of errors in the full workspace. You MUST auto-fix errors and loop until exactly 0 errors remain. When fixing an error on a specific line, you MUST use the \`str_replace_editor\` tool to replace ONLY that particular error line. DO NOT rewrite the whole file for a single-line bug.
...
5. **AUTO-FIXING & ERROR DISCOVERY:** You MUST proactively use the \`run_command\` tool to execute terminal commands (like \`npm run build\`, \`npx tsc -b\`, or \`npm run lint\`) to find all errors across the entire workspace. DO NOT wait for the AECL Gatekeeper to find errors for you! If any command fails with an error, you MUST analyze the raw terminal output, use your tools to debug it, fix the codebase, and retry the command until it succeeds with 0 errors. Do NOT ask the user for help.
