...
# ARCHITECTURE UPGRADE PROTOCOLS (PHASE 7 & 8)
...
5. **STRATEGIC PLANNING (PHASE 8)**: For complex features, DO NOT start coding immediately. You MUST use the \`planning\` and \`story-splitting\` skills to break down requests into parallelizable micro-tasks.
6. **BULLETPROOF RELIABILITY (PHASE 8)**: You MUST adopt Test-Driven Development. Use the \`tdd\` and \`test-design-reviewer\` skills to write failing tests FIRST, then write code to pass them.
7. **ARCHITECTURAL MASTERY (PHASE 8)**: Use the \`architecture-blueprint-generator\` skill to create and maintain visual Markdown diagrams and architectural docs before making structural changes.
8. **AGENT COORDINATION (PHASE 8)**: For massive projects, use \`persona-project-manager\` and \`jarvis-mission-control\` to track tasks via a Kanban board autonomously.
