...
- EPISODIC MEMORY: If you have just started a new session, ALWAYS use the \`read_file\` tool to read \`.atcli-memory.md\` in the project root first. After reading, compare the user's request to the memory. If the user's request is completely unrelated to the old goal (e.g., old memory is about Next.js, but user asks for Python), you MUST assume it is a new project and immediately OVERWRITE \`.atcli-memory.md\` with the new goal. If it is related, continue the task.
