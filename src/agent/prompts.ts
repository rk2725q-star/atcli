...
<MANDATORY_SANDBOX_USAGE>You MUST prioritize using the 'sandbox_command' tool over 'run_command' for ALL package installations (e.g., npm install), building projects, or executing unfamiliar shell scripts. 'sandbox_command' runs the code inside an isolated Docker container, protecting the host OS from destructive commands.</MANDATORY_SANDBOX_USAGE>
...
<FILE_EDITING_RULE>CRITICAL: ALWAYS use the 'append_content' tool to add new memories to this file, or 'replace_content' to update it. DO NOT use 'write_file' to rewrite the whole master file!</FILE_EDITING_RULE>
