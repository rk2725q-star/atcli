...
export function buildWorkspaceGateMessage(analyzeResult: string, cwd: string): string {
    ...
    return [
        ...
        errorCtx ? `\n${errorCtx}` : '',
        ``,
        `⛔ RULE (ALL PROVIDERS): DO NOT use write_file or delete_file to rewrite existing files.`,
        `✅ REQUIRED for each error:`,
        `  1. STOP AND REFLECT: Read the error. Is the root cause in THIS file, or in an imported dependency?`,
        `  2. If the root cause is a cross-file dependency (e.g. imported function signature changed), fix the upstream file FIRST.`,
        `  3. Identify the exact file:line from the error above`,
        `  4. read_file "<file>" — see full content if needed`,
        `  5. replace {"old": "<exact broken line>", "new": "<fixed line>"}`,
        `  6. Re-run workspace_analyze to confirm 0 failures`,
        `NEVER bypass these rules. Only modify the exact broken line(s) or fix the root dependency.`,
        `</tool_result>`,
    ].join('\n');
}
...
export function buildAeclGateMessage(mem: any): string {
    ...
    return [
        ...
        ``,
        `⛔ FORBIDDEN (ALL PROVIDERS): write_file or delete_file on existing files to bypass edits.`,
        `✅ Fix each error:`,
        `  Step 1: STOP AND REFLECT: Is the error caused by a local typo or an imported dependency?`,
        `  Step 2: If it's a cross-file dependency, fix the imported upstream file FIRST.`,
        `  Step 3: read_file "<file from error>"`,
        `  Step 4: replace {"old": "<exact broken line>", "new": "<fixed line>"}`,
        `  Step 5: aecl_check — confirm fixed`,
